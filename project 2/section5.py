    ...
    def learn(self, trajectory, nb_epoch):
        for i in range(nb_epoch):
            ...
            for j, state in enumerate(trajectory):
                ...
                td_loss = torch.abs(target - output)
                loos_array[j] = td_loss[0][0]
                self.optimizer.zero_grad()
                td_loss.backward()
                self.optimizer.step()
            print(i, '\tmean_loss = ', np.mean(loos_array))
...
def discret_PMQ(pqm_model, resolution):
...
if __name__ == "__main__":
    domain = domain()
    agent = agent_random()
    trajectory = offline_2(300, domain, agent)
    # fqi = FQI(domain, trajectory, 2)
    # J = fqi.learning_curve(100)
    # plot_J(J)
    pmq = PMQ(3)
    pmq.learn(trajectory, 10)
    agent_pmq = agent_PMQ(pmq)
    Q_dis = discret_PMQ(pmq, 0.01)
