...
def make_gif(frame_folder):
    files = glob.glob(frame_folder + "/*.jpg")
...
if __name__ == "__main__":
    ...
    while not domain.terminal_state(s):
        pos = s[0]
        speed = s[1]
        name_file = "gif/car" + str(i) + ".jpg"
        save_caronthehill_image(pos, speed, out_file=name_file)
        a = agent.chose_action(s)
        s = domain.dynamic(s, a)
        i += 1
    print("It took " + str(time.time() - t) + " seconds to generate" + str(i) + "images")
    make_gif("gif")
